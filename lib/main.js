      ...
      lint: async (textEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-packer-validate.packerExecutablePath'), args, { cwd: dir, env: packerEnv, ignoreExitCode: true }).then(output => {
          ...
          output.split(/\\r\\n\\n|\\n\\n/).forEach((line) => {
            ...
            if ((matchesSeverity != null) && (matchesSeverity[1] === 'say')) return toReturn;
            // check for issue severity
            if (matchesSeverity != null) severity = matchesSeverity[1];
            // check for issue line info
            else if (matchesFileLine != null) {
              if (matchesFileLine[1] != null) theFile = matchesFileLine[1];
