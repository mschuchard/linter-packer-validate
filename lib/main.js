  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        // TODO: activeEditor.getGrammar().toString() == 'json' or similar would be nice, but the getter returns some ambiguous object
        if (!(/"builders"\s*:\s*\[/.exec(activeEditor.getText())) && !(/source\s\".*\"\s\".*\"\s{/.exec(activeEditor.getText())))
          return [];
