  ...
  provideLinter() {
    return {
      ...
      lint: async (textEditor) => {
        ...
        // setup args
        let args = ['validate', '-machine-readable'];
        ...
        return helpers.exec(atom.config.get('linter-packer-validate.packerExecutablePath'), args, {cwd: dir, ignoreExitCode: true}).then(output => {
          ...
          output.split(/\\r\\n\\n|\\n\\n/).forEach((line) => {
            ...
            const matchesSeverity = /\d+,,ui,(say|warning|error),/.exec(line);
            ...
            // packer now outputs message on valid configurations
            if ((matchesSeverity != null) && (matchesSeverity[1] === 'say'))
              return toReturn;
            // check for issue severity
            if (matchesSeverity != null)
              severity = matchesSeverity[1];
            // check for issue line info
            ...
            // use excerpt info with previous iterations' severity, file, and line
            else if (matchesExcerpt != null || matchesExcerptAsterisk != null) {
              // determine which regexp matcher actually contains the excerpt
              let theExcerpt = matchesExcerpt == null ? matchesExcerptAsterisk[1] : matchesExcerpt[1];
              ...
              if (matchesFileLineComma != null) {
                theFile = matchesFileLineComma[1];
                row = Number.parseInt(matchesFileLineComma[2]);
                colBegin = Number.parseInt(matchesFileLineComma[3]);
                colEnd = Number.parseInt(matchesFileLineComma[4]);
