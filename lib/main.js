      ...
      lint: async (textEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-packer-validate.packerExecutablePath'), args, {cwd: dir, ignoreExitCode: true}).then(output => {
          ...
          output.split(/\\r\\n\\n|\\n\\n/).forEach((line) => {
            ...
            const matches_excerpt = regex_excerpt.exec(line);

            // check for issue
            if (matches_error != null)
              severity = matches_error[1].toLowerCase();
            else if (matches_file_line != null) {
              theFile = matches_file_line[1];
              row = Number.parseInt(matches_file_line[2])
            }
            // use excerpt info with previous iterations' severity, file, and line
            else if (matches_excerpt != null) {
              toReturn.push({
                severity: severity,
                excerpt: matches_excerpt[1].replace('%!(PACKER_COMMA)', ','),
                location: {
                  file: theFile,
