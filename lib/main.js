  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        if ((activeEditor.getGrammar().scopeName == 'source.json') && !(/"builders"\s*:\s*\[/.exec(activeEditor.getText())))
          return [];
        ...
        if ((activeEditor.getGrammar().scopeName == 'source.hcl') && !(/\.pkr\.hcl/.exec(file)))
          return [];
        ...
        return helpers.exec(atom.config.get('linter-packer-validate.packerExecutablePath'), args, {cwd: require('path').dirname(file), ignoreExitCode: true}).then(output => {
          ...
          // add info if this is json template without proper extension
          if ((activeEditor.getGrammar().scopeName == 'source.json') && !(/\.pkr\.json/.exec(file))) {
            toReturn.push({
              severity: 'info',
              excerpt: 'Packer expects JSON templates to have a ".pkr.json" extension.',
              location: {
                file: file,
                position: [[0, 0], [0, 1]],
              },
          ...
          output.split(/\r?\n|\\n/).forEach((line) => {
            ...
            // notify of JSON error
              ...
              if (activeEditor.getGrammar().scopeName == 'source.hcl') {
                toReturn.push({
                  severity: 'warning',
